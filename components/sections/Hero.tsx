...
export default function Hero() {
  ...
  return (
    <section
      className="relative min-h-[100svh] flex flex-col justify-center overflow-hidden px-6 pt-28 pb-24"
      style={{ maxWidth: "72rem", margin: "0 auto" }}
      aria-label="Hero"
    >
      {/* Orbs */}
      <div ref={orb1Ref} className="gradient-orb absolute pointer-events-none"
        style={{ width: "700px", height: "600px", top: "40%", left: "40%", transform: "translate(-50%, -55%)", background: "radial-gradient(circle at 40% 40%, #9D6FFF 0%, #7B5EA7 35%, transparent 70%)", opacity: 0.13 }} aria-hidden="true" />
      <div ref={orb2Ref} className="gradient-orb absolute pointer-events-none"
        style={{ width: "400px", height: "350px", top: "5%", right: "0%", background: "radial-gradient(circle, #B490FF 0%, transparent 65%)", opacity: 0.08 }} aria-hidden="true" />
      ...
      <div className="relative z-10 max-w-5xl">
        ...
        <h1 ref={headlineRef} className="mb-10 overflow-hidden"
          style={{ fontFamily: "var(--font-display), 'Space Grotesk', system-ui, sans-serif", fontWeight: 700, fontSize: "clamp(3rem, 8.5vw, 7rem)", lineHeight: 1.0, letterSpacing: "-0.035em" }}>
          ...
          <span className="block overflow-hidden mt-1">
            <span className="reveal-line block" style={{ opacity: 0, color: "var(--text-body)", fontWeight: 500, fontSize: "clamp(1.1rem, 3vw, 2rem)", letterSpacing: "-0.02em", marginTop: "0.25em" }}>
        ...
        <motion.div className="flex flex-wrap gap-8 mb-12"
          ...
          {STATS.map((s, i) => (
            <div key={i} className="flex items-baseline gap-1.5">
              <span style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "clamp(1.5rem, 3.5vw, 2.25rem)", letterSpacing: "-0.03em", color: "var(--text-primary)", lineHeight: 1 }}>
                {s.value}
              </span>
              {s.unit && (
                <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.75rem", color: "var(--accent-vivid)", letterSpacing: "0.05em" }}>
                  {s.unit}
                </span>
              )}
      ...
      <motion.div className="absolute bottom-8 left-8 flex items-center gap-3"
        ...
        <span className="type-label" style={{ fontSize: "0.625rem", letterSpacing: "0.14em" }}>scroll to explore</span>
      </motion.div>
    </section>
  );
}
  ...
  return (
    <button ref={ref} onClick={onClick}
      onMouseMove={(e) => {
        ...
        gsap.to(ref.current, { x: (e.clientX - r.left - r.width / 2) * 0.22, y: (e.clientY - r.top - r.height / 2) * 0.22, duration: 0.3, ease: "power2.out" });
      }}
      onMouseLeave={() => gsap.to(ref.current, { x: 0, y: 0, duration: 0.5, ease: "elastic.out(1,0.4)" })}
      style={{ padding: "13px 28px", borderRadius: "6px", background: "linear-gradient(135deg, #7B5EA7, #9D6FFF)", color: "white", fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "14px", letterSpacing: "0.01em", border: "none", boxShadow: "0 0 28px rgba(155,111,255,0.3), inset 0 1px 0 rgba(255,255,255,0.15)", transition: "box-shadow 0.2s ease", cursor: "none" }}
      onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 0 44px rgba(155,111,255,0.5), inset 0 1px 0 rgba(255,255,255,0.15)"; }}
    >
      {children}
    </button>
  );
...
function GhostBtn({ children, onClick }: { children: React.ReactNode; onClick?: () => void }) {
  ...
  return (
    <button ref={ref} onClick={onClick}
      ...
      onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--border-accent-strong)"; e.currentTarget.style.color = "var(--accent-electric)"; }}
      style={{ padding: "12px 24px", borderRadius: "6px", background: "transparent", border: "1px solid var(--border-accent)", color: "var(--text-primary)", fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "14px", letterSpacing: "0.01em", transition: "border-color 0.15s, color 0.15s", cursor: "none" }}
    >
      {children}
    </button>
  );
